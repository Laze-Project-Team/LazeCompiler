...
L_tokenList L_Lexer(const char* filename1, const char* filename2)
{
    ...
    std::wstringstream wprogramStream(program);
    std::wstring programLine = L"";
    int linePos = 0;
    while(std::getline(wprogramStream, programLine)){
        linePos += 1;
    ...
    for(const auto &regex:regexMap){
        ...
        // std::cout << converter.to_bytes(program) << std::endl;
    }
