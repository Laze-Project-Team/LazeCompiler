...
{
    T_fundec p = (T_fundec)checked_malloc(sizeof(*p));
    p -> params = params;
    p -> locals = locals;
    p -> result = result;
    p -> body = body;
...
{
    T_module p = (T_module)checked_malloc(sizeof(*p));
    p -> kind = T_func;
    p -> u.func = func;
    return p;
}
...
T_module T_ExportMod(string name, int exportt)
{
    T_module p = (T_module)checked_malloc(sizeof(*p));
    p -> kind = T_export;
    p -> u.exportt.name = name;
    p -> u.exportt.exportt = exportt;
    return p;
}
...
{
    T_moduleList p = (T_moduleList)checked_malloc(sizeof(*p));
    p -> head = head;
    p -> tail = tail;
    return p;
}
...
{
    T_typeList p = (T_typeList)checked_malloc(sizeof(*p));
    p -> head = head;
    p -> tail = tail;
    return p;
}