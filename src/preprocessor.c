...
{
    Pre_preprocessor p = (Pre_preprocessor)checked_malloc(sizeof(*p));
    p -> kind = Pre_include;
...
{
    Pre_argument p = (Pre_argument)checked_malloc(sizeof(*p));
    p -> kind = Pre_string;
    p -> u.str = val;
    return p;
}
...
{
    Pre_preprocessorList p = (Pre_preprocessorList)checked_malloc(sizeof(*p));
    p -> head = head;
    p -> tail = tail;
    return p;
}